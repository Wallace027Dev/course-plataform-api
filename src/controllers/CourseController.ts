...
export class CourseController {
  static async listCourses(req: Request, res: Response): Promise<any> {
    try {
      ...
      if (courses?.length === 0) return HttpResponse.notFound(res, "No courses found");

      return HttpResponse.ok(res, "Courses found", courses);
    } catch (error: any) {
      return HttpResponse.serverError(res, "Error while listing courses", error.message);
    }
  }
  ...
  static async getCourse(req: Request, res: Response): Promise<any> {
    try {
      ...
      return HttpResponse.serverError(res, "Error while getting course", error.message);
    }
  }
  ...
  static async getStudentsOfCourse(req: Request, res: Response): Promise<any> {
    try {
      ...
      return HttpResponse.serverError(res, "Error while getting student", error.message);
    }
  }
  ...
  static async storeCourse(req: Request, res: Response): Promise<any> {
      try {
        ...
        return HttpResponse.serverError(res, "Error while registering course", error.message);
      }
    }
